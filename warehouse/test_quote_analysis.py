...
class AnalysisDataTests(TransactionTestCase):
    ...
    def test_pickup_dates_include_all_leads_but_respect_currency_and_permissions(self):
        ...
        data = self.analyze()
        self.assertEqual(data["rows"][0]["samples"], 3)
        self.assertEqual(data["rows"][0]["address"], "Newark 07101")
        self.assertEqual(data["diagnostics"]["partial_excluded"], 0)
        self.assertEqual(self.analyze(include_partial="0")["rows"][0]["samples"], 2)
