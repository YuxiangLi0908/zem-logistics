...
class ReceivableAccounting(View):
    ...
    template_confirm_entry = "receivable_accounting/account_confirm/confirm_entry.html"
    template_confirm_transfer_edit = "receivable_accounting/account_confirm/confirm_transfer_entry.html"
    ...
    template_invoice_items_edit = "receivable_accounting/account_confirm/invoice_items_edit.html"
    ...
    def handle_supplement_order_post(
        ...
        # 其他阶段补开账单，都要先开是否已有账单未完成，没有的情况下才能补开一份账单
        ...
        if not invoices.exists():
            ...
            elif 'finance_status' in status:
                context = self.handle_payout_entry_post(request, context)
                return self.template_payout_entry, context
            else:
    ...
    def handle_container_fix_cacl_get(
        self, request: HttpRequest
    ) -> tuple[Any, Any]:
        '''柜子按报价表进行完整计算'''
        context = {}
    ...
    def _auto_calculate_delivery_fee(self, request: HttpRequest, order: Order, quotation, context, match, invoice: Invoicev2, container: Container, is_combina) -> None:
        '''派送费用的自动计算'''
        # 获取板子数据
        base_query = Pallet.objects.filter(container_number=order.container_number).exclude(PO_ID__isnull=True, PO_ID="").exclude(delivery_method__icontains="暂扣")
        ...
        if not base_query.exists():
            ...
            packing_query = PackingList.objects.filter(container_number=order.container_number).exclude(PO_ID__isnull=True, PO_ID="").exclude(delivery_method__icontains="暂扣")
        ...
        if not is_from_packing_list:
            ...
            if other_packing_query.exists():
                ...
                other_groups = list(other_groups_dict.values())
    ...
    def _process_fix_unbilled_groups(
        ...
        if combina_key not in fee_details:
            raise ValueError(f"未找到组合柜报价表规则 {combina_key}")
        ...
        if is_combina:
            ...
            for candidate in combina_candidates:
                ...
                # 计算费用
                price_base = price_item["prices"][container_type_temp]
            ...
            non_combina_groups = pallet_groups
        # 处理未录入的PO
    ...
    def handle_fix_account_entry_post(self, request:HttpRequest, context: dict| None = None,) -> Dict[str, Any]:
        ...
        # --- 6. 循环处理 ---
        order_data_list = []      # 待开账单
        previous_order_data_list = [] # 已开账单 
        ...
        for o in base_orders:
            ...
            if not invoices:
                ...
                for invoice in invoices:
                    ...
                    invoice_created_at = invoice.created_at if has_multiple_invoices else None
    ...
    def handle_write_off_entry_post(self, request:HttpRequest, context: dict| None = None,) -> Dict[str, Any]:
        '''已核销账单查询'''
        if not context:
            context = {}
        warehouse = request.POST.get("warehouse_filter")
        ...
        for o in base_orders:
            ...
            for invoice in invoices:
                ...
                if (preport_status == "completed" and 
                    ...
                    write_off_data_list.append(row_data)
        ...
        return self.template_write_off_entry, context
        ...
        for order in orders_list:
            ...
            if not container_invoices:
                ...
                for invoice in container_invoices:
                    ...
                    if finance_status == 'completed':
                        confirm_list.append(base_data)
                    else:
                        unconfirm_list.append(base_data)
