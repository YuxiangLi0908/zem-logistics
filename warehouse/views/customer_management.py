...
class CustomerManagement(View):
    ...
    def get(self, request: HttpRequest, **kwargs) -> HttpResponse:
        ...
        if step == "update":
            ...
            if self._validate_user_customer_balance(request.user):
                return render(
                    request,
                    self.template_balance,
                    self.handle_balance_customer_get(request),
                )
            else:
    ...
    def handle_adjust_balance(self, request: HttpRequest) -> tuple[Any, Any]:
        ...
        if transaction_type == "recharge":
            customer.balance = (
                (customer.balance + amount) if customer.balance is not None else amount
            )
    ...
    def _upload_image_to_sharepoint(self, conn, image) -> None:
        ...
        file_path = os.path.join(
            SP_DOC_LIB, f"{SYSTEM_FOLDER}/transactions/{APP_ENV}"
        )  # 文档库名称，系统文件夹名称，当前环境
        # 上传到SharePoint
