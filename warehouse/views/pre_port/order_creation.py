...
class OrderCreation(View):
    ...
    template_repeat_all = (
        "pre_port/repeat_t49_all/repeat_all.html"
    )
    ...
    async def handle_order_basic_info_get(self) -> tuple[Any, Any]:
        ...
        orders = await sync_to_async(list)(
            Order.objects.select_related(
                ...
                & (models.Q(offload_id__offload_at__isnull=True)|models.Q(offload_id__offload_other_at__isnull=True))
    ...
    async def handle_order_management_list_get(
        ...
        if start_date_eta and end_date_eta:
            criteria = (
                    models.Q(vessel_id__vessel_eta__gte=start_date_eta, vessel_id__vessel_eta__lte=end_date_eta, order_type__ne="一件代发") |
                    models.Q(created_at__gte=start_date_eta, created_at__lte=end_date_eta, order_type__ne="一件代发")
            )

        if start_date_etd and end_date_etd:
            etd_q = models.Q(vessel_id__vessel_etd__gte=start_date_etd, vessel_id__vessel_etd__lte=end_date_etd, order_type__ne="一件代发")
    ...
    async def repeat_all(self) -> tuple[Any, Any]:
        """
        分两组查询：
        柜号订单 未拆柜订单
        """
        common_filter = (
                models.Q(created_at__gte=timezone.make_aware(datetime(2024, 8, 19)))
                & (models.Q(offload_id__offload_at__isnull=True)|models.Q(offload_id__offload_other_at__isnull=True))
                & models.Q(offload_id__offload_other_at__isnull=True)
                & models.Q(cancel_notification=False)
        )
        ...
        duplicate_orders = await sync_to_async(list)(
            ...
            .order_by("container_number__container_number", "-created_at")
        )
