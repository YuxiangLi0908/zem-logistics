...
class PrePortTracking(View):
    ...
    async def download_t49_tracking_file(self, request: HttpRequest) -> HttpResponse:
        ...
        orders = await sync_to_async(list)(
            Order.objects.select_related("vessel_id", "container_number")
            .filter(models.Q(container_number__container_number__in=container_number))
            .values(
                "vessel_id__master_bill_of_lading",
                "vessel_id__shipping_line",
                "container_number__container_number",
        ...
        response["Content-Disposition"] = (
            f"attachment; filename=Terminal49_SampleCSV_Template.csv"
        )
        df.to_csv(path_or_buf=response, index=False)
        return response

    async def handle_t49_tracking_upload_post(
                ...
                if o.container_number.container_number in t49_container_numbers:
                    try:
                        o.vessel_id.vessel_eta = (
                            ...
                            if df.loc[
                                df["Container"] == o.container_number.container_number,
                                "POD Arrival",
                            ].any()
                        ...
                        packing_list = await sync_to_async(list)(
                            PackingList.objects.filter(
                                container_number__container_number=o.container_number.container_number
                            )
                        )
                    ...
                    try:
                        ...
                        pass
                    try:
                        o.retrieval_id.empty_returned = df.loc[
                            df["Container"] == o.container_number.container_number,
                            "Empty Returned",
                        ].any()
                    except:
