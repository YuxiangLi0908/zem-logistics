...
class PO(View):
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        if step == "search":
            ...
            return render(
                request, self.template_main, self.handle_selection_post(request)
            )
        elif step == "export_po":
    ...
    async def handle_upload_po_invalid_post(self, request: HttpRequest) -> tuple[Any]:
        ...
        if selected:
            for i in range(len(selected)):
                pochecketaseven = await sync_to_async(PoCheckEtaSeven.objects.get)(
                    id=selected[i]
                )
                ...
                if pochecketaseven.fba_id != fba_id[i]:
                    ...
                    packing_list = await sync_to_async(
                        lambda: pochecketaseven.packing_list
                    )()
                    if packing_list:
        ...
        if form.is_valid():
            ...
            for bol, mark, fba, ref, check_id, is_valid in data_pairs:
                if bol:
                    ...
                    try:
                        pochecketaseven = await sync_to_async(
                            PoCheckEtaSeven.objects.get
                        )(query)
                    except:
                    ...
                    if "eta" in time_code:
                        ...
                        pochecketaseven.last_retrieval_status = (
                            True if is_valid.lower() == "yes" else False
                        )
                        if "restore" in is_valid:
    ...
    async def handle_po_check_seven(
        self, request: HttpRequest, flag: str
    ) -> dict[str, dict]:
        today = datetime.now()
        if "eta" in flag:
            query = (
    ...
    async def handle_search_eta_seven(
        ...
        for order in orders:
            ...
            pls = await sync_to_async(list)(
                PackingList.objects.filter(container_number=order.container_number)
            )
            for pl in pls:
                try:
                    # 直接在查询集中查找是否存在具有相同container_number的对象
                    existing_obj = await sync_to_async(PoCheckEtaSeven.objects.get)(
                        packing_list=pl
                    )
                    if existing_obj.time_status:
                        ...
                        if (
                            order.retrieval_id
                            and order.retrieval_id.target_retrieval_timestamp
                        ):
                            ...
                            if not order.retrieval_id.actual_retrieval_timestamp:
                                ...
                                today = (
                                    datetime.now()
                                )  # 如果不筛选实际提柜早于明天，就会把港后的柜子全放进来
                                local_tz = pytz.timezone("Asia/Shanghai")
                                actual_ts = order.retrieval_id.actual_retrieval_timestamp.astimezone(
                                    local_tz
                                ).replace(
                                    tzinfo=None
                                )
                    ...
                    if (
                        pl.ref_id is not None
                        and "/" not in pl.ref_id
                        and "" not in pl.ref_id
                    ):  # 所以如果ref_id没有，就是私人地址、UPS什么的
                        # await sync_to_async(print)('需要进行拿约')
                        time_status = 0
                        if (
                            order.retrieval_id
                            and order.retrieval_id.target_retrieval_timestamp
                        ):  # 因为存在首次建表的情况，此时有的数据属于到港前一周，有的属于提柜前一天
                            # await sync_to_async(print)("有提柜计划")
                            if not order.retrieval_id.actual_retrieval_timestamp:
                                # await sync_to_async(print)('没有实际提柜')
                                po_check_dict = {
                        ...
                        if (
                            order.retrieval_id
                            and not order.retrieval_id.target_retrieval_timestamp
                        ):
                            # await sync_to_async(print)('没有实际提柜')
                            po_check_dict = {
                                "container_number": container_number,
    ...
    def handle_search_post(self, request: HttpRequest) -> dict[str, Any]:
        warehouse = (
            None
            if request.POST.get("name") == "N/A(直送)"
            else request.POST.get("name")
        )
        try:
    ...
    def _get_pl_agg_summary(self, agg_pl: Any) -> dict[str, Any]:
        if len(agg_pl) == 2:
            est_pallet = agg_pl[1]["total_n_pallet_est"] // 1 + (
                1 if agg_pl[1]["total_n_pallet_est"] % 1 >= 0.45 else 0
            )
            return {
            ...
            est_pallet = agg_pl[0]["total_n_pallet_est"] // 1 + (
                1 if agg_pl[0]["total_n_pallet_est"] % 1 >= 0.45 else 0
            )
            return {
