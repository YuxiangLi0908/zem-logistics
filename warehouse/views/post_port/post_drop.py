...
class PostDrop(View):
    ...
    async def post(self, request: HttpRequest, **kwargs) -> None | HttpResponse | tuple[Any, Any] | Any:
        ...
        if step == "ltl_post_warehouse":
            ...
            return await sync_to_async(render)(request, template, context)
        elif step == "cancel_outbound_after_confirm":
            # 取消已出库批次
            template, context = await self.handle_cancel_shipment_after_confirm(request)
            return await sync_to_async(render)(request, template, context)
        else:
            raise ValueError('wrong step',step)
    ...
    async def handle_cargo_debug(self, request: HttpRequest) -> tuple[str, dict[str, Any]]:
        ...
        def get_cargo_info():
            orders = Order.objects.filter(container_number__container_number__icontains=container_number).select_related('retrieval_id', 'container_number')
            if not orders.exists():
                return {'not_found': f'未找到柜号包含 "{container_number}" 的订单记录'}
            
            order_ids = orders.values_list('id', flat=True)
            cargos = DropshipCargo.objects.filter(order_id__in=order_ids).select_related('warehouse', 'order__retrieval_id')
            
            cargo_list = []
            for cargo in cargos:
                cargo_order = cargo.order
                retrieval_ok = False
                if cargo_order and cargo_order.retrieval_id:
                    retrieval_ok = (cargo_order.retrieval_id.planned_release_time is not None or 
                                   cargo_order.retrieval_id.temp_t49_available_for_pickup)
                ...
                cargo_list.append({
                    ...
                    'order_id': cargo_order.id if cargo_order else None
                })
    ...
    async def handle_cancel_shipment_after_confirm(
            self, request: HttpRequest, context: dict | None = None,
    ) -> tuple[str, dict[str, Any]]:
        '''取消已出库批次 - 还原确认出库时的所有操作'''
        if not context:
            context = {}
        ...
        for detail in details:
            ...
            if inventory:
                restored_pcs = inventory.after_pcs
                if cargo.pcs != restored_pcs:
                    cargo.pcs = restored_pcs
                    await sync_to_async(cargo.save)()

                if cargo.status == 'all_out':
                    cargo.status = 'in_stock'
                    await sync_to_async(cargo.save)()

                inventory.is_verify = False
                inventory.verfiy_pcs_change = 0
                inventory.verify_pcs = inventory.after_pcs
                await sync_to_async(inventory.save)()
            else:
    ...
    async def handle_drop_account_search(
            self, request: HttpRequest
    ) -> tuple[str, dict[str, Any]]:
    ...
    async def handle_account_edit_fee(
        ...
        逻辑：如果已有该类费用则直接读取显示；否则读取报价表作为参考。"""
            ...
            if fee_type == "preport":
                if dropship_quote:
                    ...
                    if preport_fee_detail and preport_fee_detail.details:
                        ...
                        template, context = await self.handle_drop_account_search(request)
                        context['error_messages'] = "报价表中未配置提拆费，请先配置报价表！"
                        return template, context
                else:
                    template, context = await self.handle_drop_account_search(request)
        ...
        total_amount = sum((item.get("amount", 0) if isinstance(item, dict) else (item.amount or 0)) for item in items)
        ...
        if fee_type == "warehouse" and invoice.container_number:
            ...
            inventory_unpack = await sync_to_async(list)(
                DropshipInventory.objects.filter(
                    cargo_id__in=cargo_ids,
                    transaction_type='unpack'
                ).select_related('cargo')
            )

            unpack_map = {inv.cargo_id: inv.transaction_date for inv in inventory_unpack}
            ...
            for detail in shipment_details_raw:
                cargo = detail.cargo
                shipment = detail.shipment
                unpack_date = unpack_map.get(cargo.id)

                storage_days = None
                storage_months = None
                if unpack_date and shipment.shipped_at:
                    storage_days = (shipment.shipped_at - unpack_date).days
                    storage_months = max(1, (storage_days + 29) // 30)
