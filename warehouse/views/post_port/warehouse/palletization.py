...
class Palletization(View):
    ...
    async def post(
        ...
        if step == "warehouse":
            ...
            template, context = await self.handle_update_warehouse_post(request)
            return render(request, template, context)
            ...
            return render(request, template, context)
        elif step == "palletize_sav_public":
            pk = kwargs.get("pk")
            template, context = await self.handle_packing_sav_public_list_post(request, pk)
            return render(request, template, context)
        elif step == "palletize_sav_other":
            pk = kwargs.get("pk")
            template, context = await self.handle_packing_sav_other_list_post(request, pk)
            return render(request, template, context)
        elif step == "back":
            template, context = await self.handle_warehouse_post(request)
            return render(request, template, context)
    ...
    async def handle_all_get_palletized(
        ...
        （2.已约未出库）
        """
        context: Dict[str, Any] = {"warehouse_form": ZemWarehouseForm()}

        if not warehouse or warehouse.strip() == "Empty":
            return self.template_pallet_warehouse_zone, context

        order_palletized: List[Order] = await self._get_order_palletized(
            warehouse.strip()
        )
        ...
        all_packing_list: List[Dict[str, Any]] = (
            await self._get_packing_list_palletized(container_number=container_numbers)
        )
    ...
    async def handle_warehouse_post(
        ...
        storehouse = request.POST.get("storehouse", "").strip()
        ...
        if ('LA' in warehouse or 'SAV' in warehouse) and storehouse == "other":
            ...
            if 'LA' in warehouse:
                ...
                context = {
                    ...
                    "order_palletized": packinglist_selfpick_cargos,
                    "order_with_shipment": order_with_shipment,
                    "warehouse_form": ZemWarehouseForm(initial={"name": warehouse}),
                    "warehouse": warehouse,
                    "storehouse": storehouse,
    ...
    async def handle_packing_sav_public_list_post(
        self, request: HttpRequest, pk: int
    ) -> tuple[str, dict[str, Any]]:
        '''sav 公仓 拆柜录入的确认'''
        order_selected = await sync_to_async(
    ...
    async def handle_packing_sav_other_list_post(
        self, request: HttpRequest, pk: int
    ) -> tuple[str, dict[str, Any]]:
        '''sav 私仓 拆柜录入的确认'''
        order_selected = await sync_to_async(
    ...
    async def handle_cancel_sav_other_post(
            self, request: HttpRequest
    ) -> tuple[str, dict[str, Any]]:
        """sav other 撤销拆柜"""
        container_number = request.POST.get("container_number")
