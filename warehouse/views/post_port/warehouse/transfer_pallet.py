...
class TransferPallet(View):
    ...
    async def post(self, request: HttpRequest, **kwargs) -> HttpRequest:
        ...
        print('transfer pallet post step',step)
        if step == "warehouse":
            ...
            template, context = await self.handle_transfer_history_warehouse_post(
                request
            )
            return render(request, template, context)
    ...
    async def handle_transfer_warehouse_post(
        ...
        fm = FleetManagement()
        await fm.insert_fleet_shipment_pallet_fleet_cost_tranfer_cost(request, fleet_number, fleet_cost)
    ...
    async def handle_transfer_history_warehouse_post(
        ...
        for t in not_arrived_raw:
            ...
            if plt_id_list:
                ...
                containers = await sync_to_async(dict)(
                    Container.objects.filter(id__in=container_ids).values_list(
                        "id", "container_number"
                    )
                )
                ...
                for p in pallets:
                    ...
                    if pair not in seen_pairs:
                        seen_pairs.add(pair)
                        pairs.append(
                            f"{containers.get(p.container_number_id, '')}-{p.destination}"
                        )
                not_arrived.append(
                    {
                        "transfer": t,
        ...
        arrived_row = await sync_to_async(list)(
            TransferLocation.objects.filter(
                receiving_warehouse=warehouse, fleet_number__arrived_at__isnull=False
            ).select_related("fleet_number")
        )
        arrived = []
    ...
    async def handle_export_bol_transfer_post(
        ...
        transfer = await sync_to_async(
            TransferLocation.objects.select_related("fleet_number").get
        )(id=transfer_id)
        fleet = transfer.fleet_number
        plt_ids = ast.literal_eval(transfer.plt_ids)
        pallets = await sync_to_async(list)(
            Pallet.objects.filter(id__in=plt_ids)
            .values("container_number__container_number", "destination", "slot")
            .annotate(total_cbm=Sum("cbm"), total_pallets=Count("id"))
            .order_by("container_number__container_number", "destination", "slot")
        )
        for _, group in groupby(
            pallets, key=lambda x: x["container_number__container_number"]
        ):
        ...
        if transfer.shipping_warehouse == "LA-91761":
            temp_pallets = []
            for container_key, container_group in groupby(
                pallets, key=lambda x: x["container_number__container_number"]
            ):
                container_list = list(container_group)

                for slot_key, slot_group in groupby(
                    container_list, key=lambda x: x["slot"]
                ):
        ...
        recv_warehouse = await sync_to_async(ZemWarehouse.objects.get)(
            name=transfer.receiving_warehouse
        )
        ship_warehouse = await sync_to_async(ZemWarehouse.objects.get)(
            name=transfer.shipping_warehouse
        )
        context = {
    ...
    async def handle_transfer_confirm_arrived_post(
        ...
        plt_ids = ast.literal_eval(transfer.plt_ids)
        pallets = await sync_to_async(list)(
            Pallet.objects.filter(id__in=plt_ids).select_related("container_number")
        )
