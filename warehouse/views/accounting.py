...
from django.db.models.fields import DecimalField, DateTimeField, CharField
...
from warehouse.models.invoicev2 import InvoiceItemv2, InvoiceStatusv2, Invoicev2
...
class Accounting(View):
    ...
    def get(self, request: HttpRequest) -> HttpResponse:
        ...
        elif step == "invoice_payable_v1":
            template, context = self.handle_invoice_payable_get_v1(request)
            return render(request, template, context)
        ...
        elif step == "inside_the_warehouse":  # 客服库内
        ...
        elif step == "invoice_warehouse_confirm_payable":  # 财务库内
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        if step == "pallet_data_search":
            ...
            return render(request, template, context)
        elif step == "invoice_order_payable_v1":
            ...
            return render(request, template, context)
        elif step == "payable_save_v1":
            template, context = self.handle_invoice_payable_save_post_v1(request)
            return render(request, template, context)
            ...
            return render(request, template, context)
        elif step == "payable_confirm_phase_v1":
            template, context = self.handle_invoice_payable_confirm_phase_v1(request)
            return render(request, template, context)
            ...
            return render(request, template, context)
        elif step == "export_delivery_by_month_carrier_v1_search":
            template, context = self.export_delivery_by_month_carrier_v1_search(request)
            return render(request, template, context)
    ...
    def handle_warehouse_entry_post(self, request: HttpRequest, context: dict | None = None, ) -> Dict[str, Any]:
        ...
        if container_number_filter:
            criteria = (Q(container_number__container_number=container_number_filter) & Q(cancel_notification=False))
        else:
            criteria = (
    ...
    def handle_confirm_entry_post(self, request: HttpRequest, context: dict | None = None, ) -> tuple[
        ...
        for o in base_orders:
            ...
            for invoice in invoices:
                # 获取预加载的状态列表
                status_list = getattr(invoice, 'payable_status_list', [])
                if not status_list:
                    continue
                ...
                if (
                    ...
                    row_data = {
                        ...
                        'retrieval_id__retrieval_carrier': getattr(o.retrieval_id, 'retrieval_carrier',
                                                                   None) if o.retrieval_id else None,
                        'retrieval_id__actual_retrieval_timestamp': getattr(o.retrieval_id,
                                                                            'actual_retrieval_timestamp',
                                                                            None) if o.retrieval_id else None,
                        'created_at': o.created_at,
                        'offload_at': o.offload_at,
                        'offload_other_at': o.offload_other_at,
    ...
    def process_orders_display_status_v1(self, orders):
        ...
        for order in orders:
            ...
            if receivable_status:
                ...
                order.display_warehouse_public_status_class = self._get_status_class(
                    receivable_status.warehouse_public_status)
                order.display_warehouse_other_status_class = self._get_status_class(
                    receivable_status.warehouse_other_status)
                order.display_delivery_public_status_class = self._get_status_class(
                    receivable_status.delivery_public_status)
                order.display_delivery_other_status_class = self._get_status_class(
                    receivable_status.delivery_other_status)
    ...
    def process_orders_display_status_v2(self, orders):
        ...
        WAREHOUSE_DELIVERY_STATUS_MAPPING = {
            "unstarted": "未录入",
            "in_progress": "录入中",
            "pending_review": "待组长审核",
            "completed": "已完成",
        ...
        for order in orders:
            ...
            if status_obj:
                ...
                raw_warehouse_public_status = status_obj.warehouse_public_status
                raw_warehouse_other_status = status_obj.warehouse_other_status
                raw_delivery_public_status = status_obj.delivery_public_status
                raw_delivery_other_status = status_obj.delivery_other_status
                # 港前提拆,仓库,派送驳回原因
                raw_preport_reason = status_obj.preport_reason
                raw_warehouse_public_reason = status_obj.warehouse_public_reason
                raw_warehouse_self_reason = status_obj.warehouse_self_reason
                raw_delivery_public_reason = status_obj.delivery_public_reason
                raw_delivery_other_reason = status_obj.delivery_other_reason
            else:
                # 港前提拆,仓库,派送状态
                raw_preport_status = None
                raw_warehouse_public_status = None
                raw_warehouse_other_status = None
                raw_delivery_public_status = None
                raw_delivery_other_status = None
                # 港前提拆,仓库,派送驳回原因
                raw_preport_reason = ""
                raw_warehouse_public_reason = ""
                raw_warehouse_self_reason = ""
                raw_delivery_public_reason = ""
                raw_delivery_other_reason = ""

            order_data = order
            if raw_preport_status != "unstarted":
                stage = STAGE_MAPPING.get(raw_preport_status, str(raw_preport_status))
                display_stage = f"提拆柜: {stage}"
            elif raw_warehouse_public_status != "unstarted":
                stage = WAREHOUSE_DELIVERY_STATUS_MAPPING.get(raw_warehouse_public_status,
                                                              str(raw_warehouse_public_status))
                display_stage = f"仓库公仓: {stage}"
            elif raw_warehouse_other_status != "unstarted":
                stage = WAREHOUSE_DELIVERY_STATUS_MAPPING.get(raw_warehouse_other_status,
                                                              str(raw_warehouse_other_status))
                display_stage = f"仓库私仓: {stage}"
            elif raw_delivery_public_status != "unstarted":
                stage = WAREHOUSE_DELIVERY_STATUS_MAPPING.get(raw_delivery_public_status,
                                                              str(raw_delivery_public_status))
                display_stage = f"派送公仓: {stage}"
            elif raw_delivery_other_status != "unstarted":
                stage = WAREHOUSE_DELIVERY_STATUS_MAPPING.get(raw_delivery_other_status,
                                                              str(raw_delivery_other_status))
                display_stage = f"派送私仓: {stage}"
            else:
                display_stage = (
                    STAGE_MAPPING.get(raw_preport_status, str(raw_preport_status))
                    if raw_preport_status
                    else "未录入任何费用"
                )

            order_data.display_stage = display_stage
            order_data.display_is_rejected = "已驳回" if (raw_preport_status == "rejected"
                                                          or raw_warehouse_public_status == "rejected"
                                                          or raw_warehouse_other_status == "rejected"
                                                          or raw_delivery_public_status == "rejected"
                                                          or raw_delivery_other_status == "rejected") \
                else "正常"
            order_data.display_reject_reason = (raw_preport_reason
                                                or raw_warehouse_public_reason
                                                or raw_warehouse_self_reason
                                                or raw_delivery_public_reason
                                                or raw_delivery_other_reason
                                                or " ")
    ...
    def handle_invoice_confirm_get_v1_delivery(
        ...
        for order in delivery_pending_orders_list:
            ...
            correlation_map[corr_id]["fleet_ids"].add(fleet_id)
            if order.fleet_number:
                correlation_map[corr_id]["fleet_numbers"].add(order.fleet_number.fleet_number or "")
                correlation_map[corr_id]["carriers"].add(order.fleet_number.carrier or "")
        ...
        for order in delivery_pending_orders_list:
            ...
            order_data = {
                ...
                "cn_total_expense": float(cn_total_expense),
                "cn_per_expense": float(cn_per_expense),
            ...
            group["total_pallets"] += cn_total_pallet
            group["total_expense"] += cn_total_expense
            group["total_rows"] += 1
            group["fleets"][fleet_id] = deliverys[fleet_id]

        final_pending = []
        for corr_id, group in correlation_groups.items():
            processed = {}
            for fid, fdata in group["fleets"].items():
                fp = {}
                for pid, pdata in fdata["fleets"].items():
        ...
        confirmed = base_delivery_orders.filter(container_number_id__in=container_ids)
        confirmed_list = list(confirmed.iterator(chunk_size=200))
        ...
        for o in confirmed_list:
            ...
            confirm_corr_map[cid]["fleet_ids"].add(fid)
            if o.fleet_number:
                confirm_corr_map[cid]["fleet_numbers"].add(o.fleet_number.fleet_number or "")
                confirm_corr_map[cid]["carriers"].add(o.fleet_number.carrier or "")
        ...
        for o in confirmed_list:
            ...
            if pn not in confirm_fleets[fid]["fleets"]:
                confirm_fleets[fid]["fleets"][pn] = {
                    "appointments": {}, "ISA_total_pallets": 0, "ISA_total_expense": Decimal("0.00"), "total_rows": 0
                }

            aid = o.appointment_id or "unknown"
            if aid not in confirm_fleets[fid]["fleets"][pn]["appointments"]:
                confirm_fleets[fid]["fleets"][pn]["appointments"][aid] = {
                    "orders": [], "total_pallets": 0, "total_expense": Decimal("0.0000"), "rowspan": 0
                }

            tp = int(o.total_pallet) if (o.total_pallet and o.total_pallet != 0) else 1
            ex = Decimal(str(o.expense or 0))
            cte = ex.quantize(Decimal("0.0000"), rounding=ROUND_HALF_UP)
            cpe = (ex / Decimal(str(tp))).quantize(Decimal("0.0000"), rounding=ROUND_HALF_UP)
            ctp = int(o.total_pallet) if o.total_pallet else 0

            od = {
                "object": o,
                "cn_total_pallet": ctp,
                "cn_total_expense": float(cte),
                "cn_per_expense": float(cpe),
                "container_num": o.container_num or "",
                ...
                "itemv2_data": {
                    ...
                    "write_off_time": o.write_off_time
                }
            }

            ad = confirm_fleets[fid]["fleets"][pn]["appointments"][aid]
            ad["orders"].append(od)
            ad["total_pallets"] += ctp
            ad["total_expense"] += cte
            ad["rowspan"] += 1
            ...
            fd["total_pallets"] += ctp
            fd["total_expense"] += cte
            fd["total_rows"] += 1

            g = confirm_groups[cid]
            g["total_pallets"] += ctp
            g["total_expense"] += cte
            g["total_rows"] += 1
            g["fleets"][fid] = confirm_fleets[fid]

        final_confirmed = []
        for cid, g in confirm_groups.items():
            proc = {}
            for fid, fd in g["fleets"].items():
                fproc = {}
                for pid, pd in fd["fleets"].items():
        ...
        start_date_export = (current_date + timedelta(days=-15)).strftime("%Y-%m-%d")
        end_date_export = current_date.strftime("%Y-%m-%d")
        pickup_carriers = {"Kars": "Kars", "东海岸": "东海岸", "ARM": "ARM", "GM": "GM", "BEST": "BEST"}
        unload_carriers = {"BBR": "BBR", "KNO": "KNO", "JOHN": "JOHN", "unload": "UNLOAD"}
        warehouse_carriers = {"BBR": "BBR", "KNO": "KNO"}
        existing_customers = Customer.objects.all().order_by("zem_name")
    ...
    def handle_invoice_payable_confirm_phase_v1(self, request):
        """ 应付财务待确认账单，驳回账单判断是提拆/库内/派送哪个阶段的 """
        confirm_phase = request.POST.get("confirm_phase")
        cotainers = request.POST.getlist("containers")
        save_type = request.POST.get("save_type")
        reject_reason = request.POST.get("reject_reason")
        ...
        for order in orders:
            ...
            if not invoicestatus:
                continue
            if save_type == "confirmed":
                invoicestatus.finance_status = "confirmed"
            elif save_type == "reject":
                invoicestatus.finance_status = "unstarted"
                invoicestatus.preport_status = "rejected"
                invoicestatus.preport_reason = reject_reason
            invoicestatus.save()
        return self._payable_confirm_get_v1(request)

    def _payable_confirm_get_v1(self, request):
    ...
    def _payable_confirm_get(self, request):
        ...
        for warehouse_pending_order in warehouse_pending_orders:
            if warehouse_pending_order["retrieval_id__retrieval_destination_area"] == "SAV" and warehouse_pending_order[
                "payable_arrive_fee"]:
                warehouse_pending_order["payable_carrier"] = "大方广"
        warehouse_confirmed_orders = (
            Order.objects.select_related(
    ...
    def export_delivery_by_month_carrier_v1_search(self, request):
        ...
        end_date_confirm = params.get("end_date_confirm")
        warehouse = params.get("warehouse_filter")
        select_carrier = params.get("select_carrier", "").strip()
        selected_customer_id = params.get("customer_name", "").strip()
        order_form = OrderForm(request.POST)
        ...
        # ========== 修复1：先获取符合条件的Order的container_number_id列表 ==========
        # 提取Order中关联的Container ID（去重）
        valid_container_ids = Order.objects.filter(criteria).values_list("container_number_id", flat=True).distinct()

        # 已确认：修复container_number__in的筛选源（用valid_container_ids而非order_exists_subquery）
        delivery_po_ids = (
            Pallet.objects.filter(container_number_id__in=valid_container_ids)  # 关键：用Container ID列表筛选
            .values_list("PO_ID", flat=True)
            .distinct()
        )
        ...
        # 仅当select_carrier非空时，才添加承运商筛选
        if select_carrier and select_carrier.strip():
            fleet_filter &= Q(fleet_number__carrier=select_carrier)
    ...
    def write_off_amount_post_delivery(self, request):
        ...
        write_off_amount_str = request.POST.get("write_off_amount")
        ...
        try:
            ...
            if write_off_amount <= 0:
                ...
                return self.handle_invoice_confirm_get_v1_delivery(
                    request, start_date_confirm, end_date_confirm, customer, warehouse_filter, error
                )
        except (ValueError, TypeError):
            return self.handle_invoice_confirm_get_v1_delivery(
        ...
        try:
            ...
            # 一车多柜：批量更新核销金额 + 核销时间
            for pallet in fleet_pallet_list:
                pallet.write_off_amount = write_off_amount
                pallet.write_off_time = current_date
                pallet.save()
    ...
    def batch_writeoff_delivery(self, request):
        ...
        if not batch_writeoff_items:
            error = "请勾选勾选框"
            return self.handle_invoice_confirm_get_v1_delivery(
                request, start_date_confirm, end_date_confirm, customer, warehouse, error
            )
        ...
        try:
            with transaction.atomic():
                for item in batch_items:
                    ...
                    if write_off_amount <= 0:
                        failed_items.append(f"车次{fleet_id}：金额无效,核销金额需要大于0")
                        continue
                    ...
                    # 一车多柜：批量更新核销金额 + 核销时间 + 财务核销备注
                    for pallet in fleet_pallet_list:
                        pallet.write_off_amount = write_off_amount
                        pallet.write_off_time = current_date
                        pallet.finance_note = note
                        pallet.save()
        except Exception as e:
            return self.handle_invoice_confirm_get_v1_delivery(
                request, start_date_confirm, end_date_confirm, customer, warehouse
            ...
            with transaction.atomic():
                ...
                for fleet_id in fleet_id_list:
                    try:
                        ...
                        # 一车多柜 → 逐个取消核销
                        for pallet in fleet_pallet_list:
                            pallet.write_off_amount = None
                            pallet.write_off_time = None
                            pallet.save()
                    except Exception as e:
    ...
    def note_post_delivery(self, request):
        ...
        # 1. 获取前端参数
        container_id_str = request.POST.get('container_id')  # 单个柜号ID
        fleet_number = request.POST.get("fleet_id")
        ...
        try:
            ...
            # 一车多柜：批量更新核销备注
            for pallet in fleet_pallet_list:
                pallet.finance_note = note_delivery
                pallet.save()
        except Exception as e:
    ...
    def export_carrier_payable_delivery(self, request):
        ...
        if confirm_phase in ['delivery', 'delivery_pending', 'delivery_confirm']:
            ...
            criteria = models.Q()
            if start_date:
                criteria &= models.Q(vessel_id__vessel_etd__gte=start_date)
            if end_date:
                criteria &= models.Q(vessel_id__vessel_etd__lte=end_date)
            ...
            # 构建FleetShipmentPallet查询条件
            fleet_shipment_criteria = models.Q(
                expense__isnull=False,
                PO_ID__in=delivery_po_ids,
            )
            ...
            for order in delivery_pending_orders:
                ...
                if hasattr(order, 'write_off_time') and order.write_off_time is not None:
                    try:
                        write_off_time_str = order.write_off_time.strftime('%Y-%m-%d %H:%M:%S')
                    except (ValueError, TypeError):
                ...
                order_data = {
                    ...
                    "cn_per_expense": per_expense,
                    "write_off_amount": write_off_amount,
                    "write_off_time": write_off_time_str,
                    "finance_note": order.finance_note or '无',
                }
                    ...
                    for appointment_id, appointment_data in pickup_data["appointments"].items():
                        for order in appointment_data["orders"]:
                            row = [
                                ...
                                order["write_off_amount"] or 0,  # 核销金额
                                order["write_off_time"] or '未核销',  # 核销时间
                                order["finance_note"] or '无',  # 备注
                            ]
    ...
    def handle_invoice_payable_save_post_v1(self, request: HttpRequest) -> tuple[Any, Any]:
        """
        单个账单点击--确认账单/返回/初级审核驳回/初级审核通过
        """
        data = request.POST.copy()
        save_type = data.get("save_type")
    ...
    def handle_invoice_payable_save_post_v1_item(self, request: HttpRequest, data, save_type, container_number) -> \
    tuple[Any, Any]:
        invoice_status = InvoiceStatusv2.objects.get(
            models.Q(invoice_type='payable') | models.Q(invoice_type='payable_direct'),
            container_number__container_number=container_number,
    ...
    def handle_container_warehouse_post(self, request: HttpRequest) -> Dict[str, Any]:
        ...
        return {
            ...
            "container_number_filter": request.GET.get("container_number_filter") or request.POST.get(
                "container_number_filter"),
            "container_number": container_number,
            "invoice": invoice,
            "invoice_number": invoice.invoice_number,
    ...
    def handle_carrier_invoice_export_v1(self, request: HttpRequest) -> HttpResponse:
        ...
        is_exclude_unload_other = not is_exclude_pickup_other
    ...
    def handle_invoice_payable_get_v1(
        ...
        return self.template_invoice_payable_v1, context
    ...
    def _all_invoice_items_get(self, invoice: Invoicev2, container_number):
        try:
            ...
            if not status_obj or status_obj.warehouse_public_status not in ("completed",
                                                                            "financials_completed") or status_obj.warehouse_other_status not in (
                    "completed", "financials_completed"):
                return {
    ...
    def handle_container_invoice_combina_get(
        ...
        if combina_region_count > combina_threshold or non_combina_region_count > (
                uncombina_threshold - combina_threshold):
            container.account_order_type = "转运"
            container.save()
    ...
    def _create_invoice_defaults_v1(self, order: Order) -> dict:
        current_date = datetime.now().date()
        order_id = str(order.id)
        customer_id = order.customer_name.id
    ...
    def _handle_direct_order_v1(self, context):
        ...
        customer_id = Customer.objects.filter(zem_name=customer).values_list('id', flat=True).first()
        # 如果已保存且不是驳回状态，从数据库读取
        if context["is_save_invoice"] and not context["is_rejected"] or context[
            "invoice_status"].preport_status == 'save':
            invoice_items = InvoiceItemv2.objects.filter(
    ...
    def _handle_regular_order(
        ...
        if act_pick_time and act_pick_time > datetime(2025, 10, 1,
                                                      tzinfo=timezone.utc) and 'LA' in retrieval_destination_precise:
            fees["basic_fee"] = 1450
        # 获取其他费用
    ...
    def handle_container_preport_post_payable(self, context: dict | None = None) -> Dict[str, Any]:
        ...
        if not existing_items.exists() and (
                invoice_status.preport_status == 'unstarted' or invoice_status.preport_status == 'rejected' or invoice_status.preport_status == 'save'):
    ...
    def _get_quotation_for_order(self, order: Order, quote_type: str = 'payable'):
        """获取订单对应的报价表"""
        try:
            vessel_etd = order.vessel_id.vessel_etd
    ...
    def _get_fee_details_from_quotation(self, quotation: QuotationMaster, fee_type: str = "PAYABLE"):
        """从报价表中获取费用详情"""
        try:
    ...
    def _get_feetail(self, actual_retrieval_timestamp, TABLENAME: str) -> Any:
        quotation = (
            QuotationMaster.objects.filter(
                effective_date__lte=actual_retrieval_timestamp, quote_type="payable"
            )
            .order_by("-effective_date")
            .first()
        )
    ...
    def handle_invoice_order_search_post_v1(
            self, request: HttpRequest, status
    ) -> tuple[Any, Any]:
        start_date = request.POST.get("start_date")
        end_date = request.POST.get("end_date")
